        ...
        <div className="w-full max-w-[1440px] sm:gap-5 bg-white mx-auto flex items-center justify-between md:gap-10 px-4 lg:px-10 py-5 xl:px-20 relative">
          <div className="flex items-center gap-2">
            <button onClick={() => setMenu(!menu)} className="lg:hidden">
              {menu ? <IoMdClose size={22} /> : <GiHamburgerMenu size={22} />}
            </button>
          ...
          <ul
            className={clsx(
              "flex flex-col lg:flex-row items-center gap-7 lg:gap-5 nav_height absolute top-[76px] left-0 lg:static w-full lg:w-auto bg-white  lg:bg-transparent py-10 lg:py-0",
              menu ? "block" : "hidden lg:flex"
            )}
          >
            <li className="">
              <Link
          ...
          <div className="w-auto md:w-full flex items-center rounded-full bg-[#f0f0f0] p-2">
            ...
            <input
              ...
              name=""
              id=""
              className="bg-transparent ml-2 outline-none hidden sm:block placeholder:text-zinc-500"
              placeholder="Search"
            />
            <div
              className={clsx(
                "w-full absolute top-[70px] left-0 bg-[#f0f0f0] px-5 py-2",
                searchOpen ? "block sm:hidden" : "hidden"
              )}
            >
              <input
                type="text"
                ...
                name=""
                id=""
                className="bg-white ml-2 outline-none rounded-full w-full px-3 py-1"
                placeholder="Search"
              />
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link
              to={clsx(cartItems.length <= 0 ? "/#" : "/cart")}
              className="relative"
            >
              <FiShoppingCart size={22} />
              <p className="text-white absolute -top-2 -right-2 bg-black text-sm rounded-full w-4 h-4 flex items-center justify-center">
