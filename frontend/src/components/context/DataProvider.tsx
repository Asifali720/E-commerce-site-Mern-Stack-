...
export interface DataProviderProps {
   ...
   addToCart: (id: string, size: string, qnt: number)=> void
   cartItems: []
...
const DataProvider = ({children}: {children: React.ReactNode}) => {
  ...
  const addToCart =(id: string, size: string, qnt: number)=>{
    const carts: any[] = []
    if(!size){
  ...
  return (
    <DataContext.Provider value={{products, currency, delivery_fee, search, setSearch, openReviews, setOpenReviews, cartItems, addToCart, setCartItems, quantity, handleQuantity, setQuantity }}>
      {children}
